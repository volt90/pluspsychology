...
> 🚧 **홈 푸터의 '마음 이야기' 링크는 지금 주석으로 막아 두었습니다** (2026-08-07, 사용자 요청).
> 되살리려면 `index.html` 푸터에서 `🚧 마음 이야기(/topics) 링크 숨김` 주석의
> 여는 표시와 닫는 표시만 지우세요.
> 페이지 7개와 `sitemap.xml` 은 그대로라 **검색에는 계속 노출됩니다.**
> 🚨 홈에서 들어가는 유일한 링크였습니다. 내부 링크가 없으면 크롤러가 덜 중요하게
> 보므로 순위가 서서히 내려갈 수 있습니다. 검색 노출까지 접으려면
> `sitemap.xml` 에서 `/topics` 주소 7개를 함께 빼고 각 페이지에 `noindex` 를 넣으세요.
> (`topics/*.html` 끼리는 서로 링크가 남아 있어 사이트 안에서는 여전히 이어집니다.)
...
## 배경음악 — 페이지를 넘어가도 이어집니다
파일 세 개가 전부입니다: `assets/bgm/bgm.js` · `bgm.css` · `landing-loop.mp3`.
페이지에는 `<head>` 에 css 한 줄, `</body>` 앞에 `<script defer>` 한 줄만 넣습니다.

- 🚨 **공개 페이지에는 빠짐없이 넣으세요.** 이 사이트는 페이지마다 별개의 HTML 이라,
  한 페이지에만 없어도 **거기 들어가는 순간 음악이 툭 끊깁니다.**
  실제로 `login`·`confirm`·`account`·`guardian`·`privacy`·`terms`·`checkout`·
  `checkout-result` 여덟 곳이 빠져 있었습니다 (2026-08-06 에 채움).
- **`admin` 에는 일부러 넣지 않았습니다** — 내부 작업 화면이라 소리가 방해가 됩니다.
- 이어붙이는 방법: 나가기 직전(`pagehide`)에 지금 재생 위치를 시각과 함께
  `sessionStorage.ksw_bgm_pos` 에 적고, 다음 페이지가 그 사이 흐른 시간만큼 앞으로 감습니다.
  페이지가 뜨는 짧은 공백은 이 구조에서 피할 수 없습니다.
- 켜짐/꺼짐 선택은 `localStorage.ksw_bgm` 에 남습니다 (방문 사이 유지). **기본값은 켜짐**이고,
  직접 끈 적이 있는 사람만 그 선택이 유지됩니다.
- 🚨 **`LOOP = 15.73771` 은 지금 음원 전용 값**입니다 (8마디 @ 122.000 BPM).
  음원을 바꾸면 이 숫자도 반드시 함께 고치세요. 안 고치면 루프 지점이 어긋나 툭툭 끊깁니다.
- `bgm.css` 는 페이지의 `:root` 색을 그대로 씁니다. 필요한 변수는
  `--line` `--ink-soft` `--blue` `--blue-deep` `--blue-soft` `--gold` 여섯 개입니다.
  **새 페이지에 넣을 때 이 변수들이 있는지 확인**하세요 (`confirm.html` 은 두 개가 없어 채웠습니다).
...
## 🚧 로그인은 지금 화면에서 숨겨져 있습니다 (2026-08-07)

회원 기능이 아직 안정적이지 않아 **헤더의 로그인 링크를 주석으로 막아** 두었습니다.
아래 절의 설명은 전부 살아 있는 코드에 대한 것이고, **입구만 닫아둔 상태**입니다.

| | 상태 |
|---|---|
| 헤더 `.acct` 로그인 링크 | 🚫 15개 공개 페이지에서 주석 처리 |
| `/login` · `/account` · `/guardian` · `/confirm` 페이지 | ✅ 그대로 동작 (주소로 직접 들어갈 수 있음) |
| `api/account.js` · Supabase | ✅ 그대로 |

- **되살리는 법**: 각 파일에서 `🚧 로그인 숨김` 주석의 여는 표시와 닫는 표시만 지우세요.
  🚨 **15개 파일에 같은 줄이 복사돼 있습니다** — `index` · `about` · `projects` · `store` ·
  `contact` · `topics` · `topics/`(6개) · `exhibitions/`(3개). 한 곳만 고치면 그 페이지에서만 보입니다.
- 🚨 **주석 문구 안에 `--` 를 쓰지 마세요.** HTML 주석이 그 자리에서 닫혀
  뒷부분이 화면에 글자로 새어나옵니다 (실제로 한 번 그렇게 만들었습니다).
- 주소를 막지는 않았습니다. 고치는 동안 직접 들어가 확인해야 하기 때문입니다.
  완전히 막아야 하면 `vercel.json` 에 redirect 를 넣으세요.
...
### 가입 실패 문구는 Supabase 스위치를 지목합니다
`authMessage()` 가 GoTrue 오류를 사람 말로 바꿉니다. **'가입이 닫혔다' 는 스위치가
두 개**라 뭉뚱그리면 어디를 켜야 할지 알 수 없어 나눠 두었습니다.

| 서버가 준 값 | 화면 문구 | 켜야 할 곳 |
|---|---|---|
| `email_provider_disabled` | 이메일 가입이 꺼져 있어요 | Authentication ▸ Sign In / Providers ▸ **Email** |
| `signup_disabled` / `Signups not allowed` | 지금은 새 가입이 닫혀 있어요 | 같은 화면 ▸ **Allow new users to sign up** |
| 429 / `over_email_send_rate_limit` | 메일 발송 한도 (남은 초까지) | Resend SMTP 연결 (기본 메일러는 시간당 몇 통) |

- 🚨 **모든 문구 뒤에 서버 원문을 괄호로 붙입니다.** 우리가 모르는 사유가 와도
  화면만 보고 진단할 수 있어야 합니다. 원문을 감추지 마세요.
...
### 확인 메일 링크가 도착하는 곳 — `/confirm`
확인 메일의 링크를 누르면 GoTrue 가 서버에서 검증한 뒤 이 주소로 돌려보냅니다.
한 파일이 **세 가지 상태**를 그립니다.

| 주소에 있는 것 | 화면 | 다음 |
|---|---|---|
| `#access_token=…` | (안 보임) | 세션을 저장하고 바로 `/account` — 인증했는데 또 로그인하는 헛걸음 제거 |
| 아무것도 없음 | 인증 완료 | 5초 뒤 `/login` 으로 자동 이동 (버튼을 누르면 즉시) |
| `error` / `error_description` | 링크 오류 | **인증 메일 다시 받기** 폼 (`/auth/v1/resend`, 60초 쿨다운) |
| `?resend=1&email=…` | 다시 받기 | 로그인 화면에서 '아직 인증 안 됨' 을 만났을 때 보내는 곳 |

- 🚨 **`<title>` 을 고정값으로 되돌리지 마세요.** 예전에는 `이메일 인증 완료` 로 박혀 있어서
  링크가 만료돼 실패 화면이 떠도 탭 제목만 '완료' 라고 우겼습니다. `TITLE` 상수가 상태를 따라갑니다.
- 🚨 **여기는 웹에서 가입한 사람이 도착하는 자리입니다.** 예전에는 `world.kimsimri.app://`
  딥링크 버튼 하나뿐이라 웹 사용자가 눌러도 아무 일도 없었습니다. 앱은 자기 브라우저 안에서
  이 주소를 열어 스스로 알아채므로, 이 화면에 앱 이야기를 다시 넣지 마세요.
- 재발송도 `redirect_to` 는 **쿼리스트링**입니다 (`login.html` 과 같은 규칙).
- ⚠️ **링크 만료 시간은 코드가 아니라 Supabase 설정입니다** —
  Authentication ▸ Emails ▸ **Email OTP Expiration** (초). 기본값 3600. 짧게 잡혀 있으면
  메일을 열기도 전에 만료됩니다.

### 그 밖에
- 세션은 `sessionStorage` 에만 둡니다 — 탭을 닫으면 로그아웃됩니다.
- 로그인 실패는 **세 갈래**입니다. `error_code` 를 먼저 보고(문구는 판올림 때 바뀝니다)
  `email_not_confirmed` 면 '인증 메일 다시 받기' 링크를 붙여 줍니다. 모르는 사유는
  서버 원문을 괄호로 함께 보여줍니다.
...
- 제목·본문·`.lede` 는 `--read` 로 **폭만** 묶습니다. 카드·그리드·사진은
  `--page` 폭을 그대로 씁니다. **컨테이너를 넓혔다고 글줄까지 길어지면 읽기 어려워집니다.**
- 🚨 **글 블록은 왼쪽 끝에 붙입니다 (`margin-left:0`). 가운데로 밀지 마세요.**
  알약(`.eyebrow`)은 `inline-flex` 라 `margin:auto` 가 듣지 않아 **항상 왼쪽 끝**에 섭니다.
  제목·본문만 가운데 열로 밀면 알약과 왼쪽 끝이 130px 어긋나 보입니다
  (2026-08-07 에 `index.html` 에서 이 상태를 고쳤습니다 — `projects`·`store` 는 이미 왼쪽이었음).
  **히어로(`.hero`)만 일부러 가운데 정렬**이고, 카드 안쪽(`.price-card`·`.cta-card`·`.friends`)도
  자기 상자 기준으로 가운데입니다. 이 셋은 건드리지 마세요.
