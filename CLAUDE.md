...
#### Home 은 '전체 폭 색 띠' 구조입니다 (다른 탭은 아직 아님)
섹션마다 배경색이 **화면 끝까지** 가고, 글만 가운데로 모입니다.

```
<section class="price on-blue">     ← 띠: 배경색을 갖고 화면 끝까지
  <div class="wrap"> … </div>       ← 안쪽: --page(960px) 폭으로 내용을 묶음
</section>
```

- 🚨 **Home 에 섹션을 추가할 때는 반드시 안에 `<div class="wrap">` 을 두세요.**
  없으면 글이 화면 끝까지 늘어납니다.
- 띠 색 클래스는 셋뿐입니다 — `on-white`(흰색) · `on-blue`(연파랑) · `on-gold`(연노랑).
  **클래스를 안 붙이면** body 의 크림 + 점무늬가 그대로 보입니다(= 크림 띠).
- 현재 순서: 히어로(크림) → 문제(흰) → 해결(크림) → 가격(파랑) → 증거(흰) →
  CTA(노랑) → 푸터(흰). **같은 색이 이웃하지 않게** 두세요. 색이 안 바뀌면 띠가 안 보입니다.
- 섹션 사이 간격은 이제 **띠의 `padding`** 이 만듭니다. `section{margin-top:…}` 은 지웠으니
  간격을 바꾸려면 `.page > section` 의 padding 을 고치세요.
- 섹션 머리(알약 → 제목 → 리드문)는 띠 가운데로 모읍니다. 카드·목록 안의 글은 왼쪽 정렬입니다.
- `.mate`(김심리 캐릭터)는 `.problem .wrap` 을 기준으로 놓입니다. `.problem` 에
  `position:relative` 를 되돌리면 캐릭터가 **화면 오른쪽 끝으로 날아갑니다.**
- About·Projects·Store·Contact 는 아직 옛 단일 컬럼(`.page` 가 폭을 잡는 방식)입니다.
  같은 구조로 옮기려면 위 두 가지(섹션에 `wrap` 넣기, `.page` 폭 풀기)를 그대로 하면 됩니다.
